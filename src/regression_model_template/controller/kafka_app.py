...
class FastAPIKafkaService:
    ...
    def _process_message(self, msg: Message) -> None:
        ...
        try:
            ...
            try:
                num_rows = len(next(iter(kafka_msg["input_data"].values()))) if kafka_msg.get("input_data") else 0
                logger.info(f"kafka Received input with {num_rows} rows")
            except Exception:
                logger.info("kafka Received input")
...
async def predict(request: PredictionRequest) -> PredictionResponse:  # Use global var
    ...
    try:
        ...
        try:
            num_rows = len(next(iter(request.input_data.values()))) if request.input_data else 0
