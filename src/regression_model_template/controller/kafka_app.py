...
async def predict(request: PredictionRequest) -> PredictionResponse:  # Use global var
    ...
    try:
        ...
        try:
            row_count_res = len(prediction_result.result.get("inference", []))
            logger.info(f"HTTP prediction result with {row_count_res} rows")
        except Exception:
            logger.info("HTTP prediction result")
