...
def get_changed_files():
    try:
        output = subprocess.check_output(['git', 'diff', 'HEAD~1', 'HEAD', '--name-only'], stderr=subprocess.STDOUT).decode('utf-8')
    ...
    try:
        output = subprocess.check_output(['git', 'diff-tree', '--no-commit-id', '--name-only', '-r', 'HEAD'], stderr=subprocess.STDOUT).decode('utf-8')
        res = [f for f in output.split('\n') if f.startswith('src/') and f.endswith('.py')]
        return res
    except subprocess.CalledProcessError:
        pass
    ...
    try:
        output = subprocess.check_output(['git', 'show', '--name-only', '--format=', 'HEAD'], stderr=subprocess.STDOUT).decode('utf-8')
        res = [f for f in output.split('\n') if f.startswith('src/') and f.endswith('.py')]
        return res
    except subprocess.CalledProcessError:
        pass
            ...
            if frontmatter_end != -1:
                     ...
                     if '```mermaid\nflowchart TD' in body:
                          ...
                          if len(parts) > 1:
                              for i in range(1, len(parts)):
                                  ...
                                  if 'subgraph ' not in block_content and '|' not in block_content and (' abc' in block_content or 'typing' in block_content or 'pydantic' in block_content or (md_name + ' -->') in block_content or 'Dependencies found' in block_content):
                ...
                if changed:
                    frontmatter = re.sub(
                        ...
                        f'last_verified_commit: "{commit_hash}"',
                        frontmatter
                    )
    ...
    if changed_files:
         for filepath in changed_files:
             if os.path.exists(filepath):
                  ...
                  if mod_name: modules.append(mod_name)
    else:
