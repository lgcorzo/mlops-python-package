...
## 2026-03-05 - Missing Security Middleware
**Vulnerability:** The FastAPI application lacked `CORSMiddleware` and `TrustedHostMiddleware`, leaving it vulnerable to Cross-Origin Resource Sharing attacks and Host Header Injection if deployed without an external gateway.
**Learning:** Default FastAPI templates often omit these middlewares, assuming they will be handled by a proxy (like Nginx), but a "defense in depth" strategy requires the application to enforce these policies itself.
**Prevention:** Always include `CORSMiddleware` and `TrustedHostMiddleware` with strict defaults (allowing configuration via environment variables) to ensure the application is secure by default in any deployment environment.
