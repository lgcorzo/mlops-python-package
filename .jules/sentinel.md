...
**Vulnerability:** The FastAPI application was lacking basic security headers (TrustedHostMiddleware) and CORS protection (CORSMiddleware), potentially exposing it to Host Header attacks and unauthorized cross-origin requests.
**Learning:** Default FastAPI configurations are minimal and do not include these protections out of the box. They must be explicitly added.
**Prevention:** Implement `TrustedHostMiddleware` and `CORSMiddleware` reading from environment variables (e.g., `ALLOWED_HOSTS`, `ALLOWED_ORIGINS`) with secure defaults or restrictive settings for production.
